...
- (void)turnOfflineViewController{
    ...
    [offlineViewController didDeleteBlock:^(DWOfflineModel *offlineModel, BOOL isDownloading,NSDictionary *dic) {
        ...
        if (isDownloading) {
            ...
            self.finishDicArray =[defaults objectForKey:@"finishDicArray"];
            self.finishDicArray =[self.finishDicArray mutableCopy];
            [self.finishDicArray removeObject:dic];
            ...
            //也要删除相应的文件
            [[DWDownloadSessionManager manager] deleteAllFileWithDownloadDirectory:[dic objectForKey:@"videoPath"]];
...
- (void)startDownloadWith:(DWOfflineModel *)model videoPath:(NSString *)videoPath isBegin:(BOOL)isBegin{
    ...
    DWDownloadModel *loadModel =[[DWDownloadModel alloc]initWithURLString:model.playurl filePath:videoPath responseToken:model.token userId:DWACCOUNT_USERID videoId:model.videoId];
    ...
    [manager startWithDownloadModel:loadModel progress:^(DWDownloadProgress *progress,DWDownloadModel *downloadModel) {
        ...
        if ([downloadModel.downloadURL isEqualToString: model.playurl]){
            ...
            if (state ==DWDownloadStateCompleted) {
                ...
                //                NSDictionary *dic =@{@"videoPath":model.videoPath,
                ...
                self.finishDicArray =[[NSUserDefaults standardUserDefaults] objectForKey:@"finishDicArray"];
                self.finishDicArray =[self.finishDicArray mutableCopy];
