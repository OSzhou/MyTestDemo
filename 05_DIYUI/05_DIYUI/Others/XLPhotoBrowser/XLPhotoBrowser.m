...
- (void)show:(UIView *)superView;
{
    ...
    self.frame = CGRectMake(0, 0, XLScreenW, XLScreenH - 234);
    self.alpha = 0.0;
...
+ (instancetype)showPhotoBrowserWithImages:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex {
    return [self fm_showPhotoBrowserWithSuperView:nil images:images currentImageIndex:currentImageIndex];
}

+ (instancetype)fm_showPhotoBrowserWithSuperView:(UIView *)superView images:(NSArray *)images currentImageIndex:(NSInteger)currentImageIndex {
    ...
    [browser show:superView];
    return browser;
}

- (void)contentView:(XLZoomingScrollView *)contentView touchesBegan:(NSSet *)touches {
    NSLog(@" --- 123456!");
    !_testBlock ?: _testBlock();
}
