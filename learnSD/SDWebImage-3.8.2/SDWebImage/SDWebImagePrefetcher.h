...
/**
    因为SDWebImage是给UIImageView写的一个Category，所以正常情况下，都是UIImageView初始化完毕后才会调用对应API去请求图片。而SDWebImagePrefetcher这个类是在UIImageView还没创建的时候就下载好图片备用，当UIImageView初始化后，请求图片，通过URL直接去缓存里读取，节省时间！！！
 */
